...
function toggleTooltips() {
  ...
  const status = document.getElementById("tooltipsStatus")
  const isEnabled = toggle.checked

  // Guardar preferencia en localStorage
  localStorage.setItem("tooltipsEnabled", isEnabled)

  // Actualizar texto del estado
  status.textContent = isEnabled ? "Activados" : "Desactivados"

  // Aplicar cambios inmediatamente si estamos en la página de Simplex
  if (typeof window.updateTooltipsState === "function") {
    window.updateTooltipsState(isEnabled)
  }
  ...
  if (window.Swal) {
    window.Swal.fire({
      ...
      position: "top-end",
      icon: "success",
      title: isEnabled ? "Tooltips activados" : "Tooltips desactivados",
...
// Cargar estado de tooltips al cargar la página
document.addEventListener("DOMContentLoaded", () => {
  const tooltipsEnabled = localStorage.getItem("tooltipsEnabled")
  const toggle = document.getElementById("tooltipsToggle")
  const status = document.getElementById("tooltipsStatus")

  if (tooltipsEnabled !== null) {
    const isEnabled = tooltipsEnabled === "true"
